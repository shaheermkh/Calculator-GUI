...
add = tk.Radiobutton(
    frame, text="Add (+)", variable=choice, value="1", command=disable_inputs
)
add.pack(anchor="w")

substract = tk.Radiobutton(
    frame, text="Subtract (-)", variable=choice, value="2", command=disable_inputs
)
substract.pack(anchor="w")

multiply = tk.Radiobutton(
    frame, text="Multiply (*)", variable=choice, value="3", command=disable_inputs
)
multiply.pack(anchor="w")

divide = tk.Radiobutton(
    frame, text="Divide (/)", variable=choice, value="4", command=disable_inputs
)
divide.pack(anchor="w")

bodmas = tk.Radiobutton(
    frame,
    text="Expression (BODMAS)",
    variable=choice,
    value="5",
    command=disable_inputs,
)
...
button.pack(pady=10)
tk.Button(
    button, text="Calculate", command=calculate, bg="green", fg="white", width=15
).pack(side="left", padx=5)
tk.Button(
    button, text="Export to CSV", command=export, bg="blue", fg="white", width=15
).pack(side="left", padx=5)
